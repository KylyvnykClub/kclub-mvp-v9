...
/**
 * The membership card, drawn in CSS over the club's emblem.
 *
 * Built rather than photographed: the gold plate that used to stand in for it
 * carried the previous mark ("KQ", "BUSINESS CLUB") and could not be
 * re-lettered without a new render, while the card here is text the browser
 * draws - so the serial, the tier and the language change with the page, and
 * the only artwork is the emblem itself.
 *
 * Black and gold are the mark's own colours (`--accent`, `#d4af37`). The emblem
 * PNG carries brightness as alpha, so it composites onto the dark card exactly
 * as it was drawn and needs no background of its own.
 */
export function MembershipCard({
  caption,
  serial = "KCLUB 000 001",
  tier = "VIP",
}: {
  caption: string;
  serial?: string;
