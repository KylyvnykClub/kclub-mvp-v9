        ...
        {payload
          ...
          .map((item, index) => {
            ...
            const indicatorColor =
              color ?? payloadFill(item.payload) ?? item.color;
...
/**
 * Recharts types `dataKey` as string | number | accessor function; only the
 * first two can name a config entry. Falls back to "value" like upstream.
 */
function toKey(...candidates: unknown[]): string {
...
/** The per-datum `fill` (from <Cell>) when the payload carries one. */
