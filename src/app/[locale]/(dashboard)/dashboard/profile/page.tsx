...
export default async function ProfilePage({ params }: Props) {
  ...
  const memberSince = member.createdAt 
    ? new Intl.DateTimeFormat(locale, { year: 'numeric', month: 'long', day: 'numeric' }).format(new Date(member.createdAt))
    : '';
    
  const cardIssuedAt = card?.issuedAt
    ? new Intl.DateTimeFormat(locale, { year: 'numeric', month: '2-digit', day: '2-digit' }).format(new Date(card.issuedAt))
    : '';
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <p className="text-sm text-muted-foreground">{tDashboard('name')}</p>
                <p className="font-medium">{member.displayName || '-'}</p>
              </div>
              <div className="space-y-1">
                <p className="text-sm text-muted-foreground">{tDashboard('phone')}</p>
                <p className="font-medium font-mono tracking-wider">{maskPhone(member.phone)}</p>
              </div>
              <div className="space-y-1">
                <p className="text-sm text-muted-foreground">{tDashboard('country')}</p>
                <p className="font-medium">{member.country || '-'}</p>
              </div>
              <div className="space-y-1">
                <p className="text-sm text-muted-foreground">{tDashboard('language')}</p>
                <p className="font-medium uppercase">{member.language || locale}</p>
              </div>
          ...
          >
            ...
            <div className="absolute inset-0 p-6 flex flex-col justify-between z-10">
              ...
              <div className="flex justify-between items-start">
                ...
                {card && (
                  <Badge variant="outline" className="bg-black/60 border-accent/60 text-accent uppercase tracking-widest text-[10px] font-bold px-2 py-0.5">
                    {card.tier === 'vip' ? tCard('tierVip') : tCard('tierFree')}
                  </Badge>
                )}
              </div>
              
              {/* Middle Row (Chip / NFC indicator) */}
              <div className="flex items-center gap-2 opacity-80 mt-2">
                <svg className="w-9 h-9 text-accent/80" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1">
                  ...
                  {card && (
                    <div className="flex flex-col items-end gap-1.5">
                      <div className="flex items-center gap-1.5">
                        <span className={`w-2 h-2 rounded-full ${card.status === 'valid' ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]' : 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.6)]'}`}></span>
                        <span className="text-[9px] uppercase tracking-[0.15em] text-muted-foreground">
                          {card.status === 'valid' ? tCard('statusValid') : tCard('statusRevoked')}
                        </span>
                      </div>
