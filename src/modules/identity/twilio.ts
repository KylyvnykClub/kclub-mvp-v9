...
export async function sendVerificationCode(phone: string): Promise<boolean> {
  if (env.server.AUTH_DEV_PHONE_BYPASS_ENABLED) {
    logger.info(`[DEV BYPASS] Pretending to send Twilio verification code to ${phone}`);
    return true;
  }

  try {
    const auth = Buffer.from(`${env.server.TWILIO_ACCOUNT_SID}:${env.server.TWILIO_AUTH_TOKEN}`).toString("base64");
    ...
    if (!response.ok) {
      ...
      logger.error(`Twilio Verify API Error (send): ${response.status} ${errorText}`);
      return false;
    }

    return true;
  } catch (error) {
    logger.error("Failed to send Twilio verification code", error);
    return false;
  }
}
...
export async function checkVerificationCode(phone: string, code: string): Promise<boolean> {
  if (env.server.AUTH_DEV_PHONE_BYPASS_ENABLED) {
    ...
    logger.info(`[DEV BYPASS] Checked code '${code}' for ${phone} - Valid: ${isValid}`);
    return isValid;
  }
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      logger.error(`Twilio Verify API Error (check): ${response.status} ${errorText}`);
      return false;
    }
    ...
    logger.error("Failed to check Twilio verification code", error);
    return false;
  }
}
