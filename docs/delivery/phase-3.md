...
|T-3.3|Listing checkout flow per company, subscription metadata bound to the owning company, ownership validated before the session is created|FR-051|T-3.1|1d|partial — session creation + ownership validation exist (findApprovedCompanyByOwner), integration test proves guard; company metadata passed in subscription_data|
