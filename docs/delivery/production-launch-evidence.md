...
|`AC-04`|Card verification discloses nothing beyond FR-023, confirmed by reviewing the actual HTTP response body.|`in_progress`|Card token hashing and integration tests are present; public route exists at `/[locale]/card/[token]`.|Capture valid, revoked, and unknown token HTTP responses from preview and review the raw bodies.|
|`AC-05`|No endpoint returns a set of members to any member-level role, confirmed by an automated test over the whole route table.|`in_progress`|Constraint suites include route/member leak tests and RBAC tests.|Confirm route registry coverage is non-empty and includes every production route before launch.|
