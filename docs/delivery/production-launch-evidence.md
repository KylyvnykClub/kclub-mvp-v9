...
|-|-|-|-|-|
|`AC-01`|All **M** functional requirements implemented, each with an automated test referencing its FR ID.|`ready_for_rehearsal`|FR-006 has an implementation and tests naming it, and since [ADR 0032](../decisions/0032-phone-and-email-both-required.md) the ordinary path is self-service: `/forgot-password` takes either identifier and emails a single-use link to the verified address, and the password change kills every other session. `tests/password-reset-self-service.integration.test.ts` covers that path, `tests/password-reset-requests.integration.test.ts` the staff queue that still serves accounts holding no verified address (a request grants nothing, one open row per member, only the first closer wins), and `tests/password-reset.integration.test.ts` the staff reset itself. `python tools/check-plan.py --strict` passes.|**Read FR-006 before signing this off**: self-service covers members who registered after ADR 0032 and verified their address. The nine accounts that predate it hold none and are still staff-recovered, and `requirements.md` §9 remains open for exactly that case.|
