...
|`AC-12`|Production SMS delivery verified end to end through Twilio Verify.|`ready_for_rehearsal`|Twilio keys and external checks are listed in [production-env-readiness.md](production-env-readiness.md#sms-twilio-verify). A2P 10DLC registration was dropped as inapplicable on 2026-08-13 — Verify sends from Twilio's registered pool, so no KCLUB sender exists to register ([ADR 0010](../decisions/0010-no-own-a2p-registration-with-twilio-verify.md)).|Confirm Fraud Guard, geographic permissions and the daily spend cap, then run a production SMS smoke to an allowlisted owner phone and record the number, timestamp and result.|
