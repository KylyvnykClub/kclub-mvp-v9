...
|-|-|-|-|-|
|`AC-01`|All **M** functional requirements implemented, each with an automated test referencing its FR ID.|`ready_for_rehearsal`|FR-006 has an implementation and tests naming it, and since [ADR 0031](../decisions/0031-identity-returns-to-phone-only.md) it is the staff-performed one: a member asks through `/forgot-password`, the request appears in the console, and a staff owner resets the password with the audit entry ADR 0018 already required. `tests/password-reset-requests.integration.test.ts` covers the queue (a request grants nothing, one open row per member, only the first closer wins) and `tests/password-reset.integration.test.ts` the reset itself. `python tools/check-plan.py --strict` passes.|**Read FR-006 before signing this off**: recovery is staff-performed, not self-service, and `requirements.md` §9 is open again. If the launch criteria intend a member to recover unaided, this row is not met.|
