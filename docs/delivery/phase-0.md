...
|T-0.11|`platform` module skeleton: `Actor`, `assertCan`, rate limiting on Upstash, outbox table and dispatcher, feature flags|—|T-0.10|1.5d|done 2026-08-04 — `Actor` discriminated union + `staffAtLeast()`, `assertCan()` rule-based authorization, six typed domain errors, outbox table with `FOR UPDATE SKIP LOCKED` drain, feature flags (5 seed rows), in-memory rate limiter + `assertRateLimit()`, migration with partial index, 42 unit tests across 5 files|
