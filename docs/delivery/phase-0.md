...
|T-0.5|The three blocking client decisions closed and recorded (see §3)|—|—|1d + client|done|
...
|T-0.14|i18n scaffolding: `next-intl`, locale-prefixed routes, `messages/{en,ru,uk}.json`, missing-key check in CI, ICU plural setup for `ru`/`uk`|FR-090, FR-091|T-0.6|1d|done 2026-08-04 — `next-intl` 4.13 with App Router, `src/i18n/` (routing, request, navigation), middleware for locale-prefixed routes, `src/app/[locale]/` layout with `NextIntlClientProvider` and `lang` attribute, messages in `messages/{en,ru,uk}.json` with ICU plurals (one/few/many/other for ru/uk), `tools/check-i18n-keys.ts` CI gate in `pnpm verify`, 6 unit tests proving plural categories and key completeness|
|T-0.15|Design tokens from [ux.md §6](../ux.md#6-design-system): Tailwind 4 custom properties, light and dark themes, shadcn/ui base installed|FR-092|T-0.6|1d|done 2026-08-04 — Tailwind 4, next-themes, globals.css OKLCH tokens, fonts and 9 shadcn/ui components added|
...
|T-0.19|Process documents corrected to the real team shape (see §3)|—|T-0.4|0.5d|done 2026-08-05|
|T-0.20|FR-090…FR-098 assigned to phases; [requirements.md §6.1](../requirements.md#61-delivery-plan) updated; `check-plan.py --strict` green|—|T-0.5|0.5d|done 2026-08-05|
|T-0.21|`phase-1.md` written|—|T-0.5, T-0.20|0.5d|done 2026-08-05|
