...
|Aspect|Approach|
|-|-|
|Method (member)|Password plus one of two identifiers: a phone number (E.164) always, or an email address the member has proved by opening a link ([ADR 0032](decisions/0032-phone-and-email-both-required.md)). Both are collected at registration and only the number is usable until the link is opened; an unproved address is refused with the same answer as a wrong password. Google sign-in exists behind a feature flag that is off; while it is off the button is absent and both OAuth routes answer 404. First registration additionally requires an SMS one-time code; later sign-ins require one only from an unrecognised device|
...
|Verification links|Emailed links are 32 random bytes; only their SHA-256 is stored, under a domain prefix of their own so a link issued for one purpose cannot be presented as another. Single-use, stamped consumed in the same statement that reads them, and redeemed by a POST rather than by opening the URL — mail scanners follow links|
...
|Confidential — member PII|Phone number, email address, display name, country, card serial, session records, IP addresses, inbox notifications, password-reset requests|AES-256 at rest (Neon); phone number additionally indexed by a keyed hash so lookups do not require scanning plaintext. The email address is held for every member registered after [ADR 0032](decisions/0032-phone-and-email-both-required.md) and is stored as typed, lowercased — normalising it further would send recovery mail to an address the member never wrote. Notifications hold no prose — a `kind` and a `params` object of ids, names and integer amounts, rendered at read time ([ADR 0020](decisions/0020-member-inbox.md)) — and never name another member|TLS 1.3|The owning member **only**: every inbox query is scoped by the session's member id, and there is no action, for any role, that reads another member's notifications|
|Confidential — member PII (object storage)|Member avatar photo|R2's own at-rest encryption; the file itself is re-encoded server-side before storage, which also discards EXIF (location, device, capture time) that the original upload may have carried ([ADR 0021](decisions/0021-member-avatar-upload.md))|TLS 1.3|The owning member only, via `/api/avatar` — the route takes no target member id, so there is no request shape that returns another member's avatar|
...
|Enumeration|Sign-in returns the same response and takes the same time whether or not the identifier is known. **Registration is the stated exception ([ADR 0030](decisions/0030-registration-says-a-number-is-taken.md)): its first step says when a phone number already belongs to a member, so an applicant is not made to fill in the whole form first. Bounded by a 20-per-hour limit on the address asking — on the first-step lookup where it is called, and on the registration submit where the same message now comes out of the unique constraint — and it discloses nothing beyond unavailability. The exception covers the number only: the email address registration now also requires is refused with "that address cannot be used", on the last step, saying nothing about who holds it ([ADR 0032](decisions/0032-phone-and-email-both-required.md)).** The card verification page returns the same shape for unknown, revoked and valid tokens. Password reset answers the same way whether or not the identifier is known, whether or not the account holds an address, and whether or not that address is verified — one reply covering both outcomes, that a link is on its way if the account holds a verified address and that support has been asked otherwise, so which branch ran is not disclosed either. The reset page itself does not check the token on load, so a visitor cannot learn whether a link is real without spending it|
|Bots|Cloudflare Turnstile on registration and on the password-reset request, invisible unless the request looks automated. Chosen over reCAPTCHA to avoid sending visitor data to an advertising company. The reset request is additionally rate limited by identifier and by address, because it sends mail to an address the caller names|
