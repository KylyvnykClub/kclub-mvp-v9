...
|Aspect|Approach|
|-|-|
|Method (member)|Phone number (E.164) + password. A member may also hold one verified email address as a second identifier ([ADR 0028](decisions/0028-email-identifier-and-account-recovery.md)); an address that has not been proved by opening a single-use link signs nobody in. First registration additionally requires an SMS one-time code; later sign-ins require one only from an unrecognised device|
...
|Confidential — member PII|Phone number, email address, display name, country, card serial, session records, IP addresses, inbox notifications|AES-256 at rest (Neon); phone number additionally indexed by a keyed hash so lookups do not require scanning plaintext. Notifications hold no prose — a `kind` and a `params` object of ids, names and integer amounts, rendered at read time ([ADR 0020](decisions/0020-member-inbox.md)) — and never name another member|TLS 1.3|The owning member **only**: every inbox query is scoped by the session's member id, and there is no action, for any role, that reads another member's notifications|
...
|Enumeration|Sign-in returns the same response and takes the same time whether or not the identifier is known. **Registration is the stated exception ([ADR 0030](decisions/0030-registration-says-a-number-is-taken.md)): its first step says when a phone number already belongs to a member, so an applicant is not made to fill in the whole form first. Bounded by a 20-per-hour limit on the address asking, and it discloses nothing beyond unavailability.** The card verification page returns the same shape for unknown, revoked and valid tokens. Password reset answers the same way whether or not the identifier is known, whether or not the account holds an address, and whether or not that address is verified — the reply says only that a link is on its way if the account exists. The reset page itself does not check the token on load, so a visitor cannot learn whether a link is real without spending it|
