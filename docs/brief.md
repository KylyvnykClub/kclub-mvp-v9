...
|-|-|-|
|Is the club's operator prepared to be the data controller for EU members' personal data? **Resolved 2026-08-05 — EU members are accepted and the operator is the controller; no Art. 27 representative is appointed.** The legal pack still has no GDPR section — the design implements the controls against a policy that does not yet promise them, see [requirements.md §5.5](requirements.md#55-compliance-and-legal)|Client + counsel|Resolved|
