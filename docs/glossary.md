...
| Company | A business record at any stage. The published statuses are fixed by Partner Rules §6: `under_review`, `published`, `hidden`, `suspended`, `removed`, plus the internal `draft` and `approved` that precede publication | `Company` | `company` | "Partner", which is a company that has completed all three gates |
