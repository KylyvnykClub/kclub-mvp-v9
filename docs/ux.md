...
|Pricing|Compare what membership, VIP and a listing cost before signing up|public|Still served and still canonical, but no longer where the main navigation's "Pricing" points: the landing carries all three amounts beside the card they buy, and that is the fuller answer, so the link goes to `/#membership`. Amounts come from `src/domain/pricing.ts` — one table for the whole product, never a price written into a translation string — in USD with an explicit `$` ([ADR 0033](decisions/0033-standard-membership-is-paid.md)). Stripe remains what actually charges: the `plan_prices` row is the price that is billed|
